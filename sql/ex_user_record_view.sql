...
`items_stay`.`stay_time` AS `item_stay_time`,`items_map`.`stay_time` AS `map_stay_time`
from (`ex_record` `record` join ((`ex_record_items` `items`
...
WHERE (`record`.`id` = `items`.`record_id` AND `items`.`times` > 1 AND
	`items_stay`.`stay_time` > 5 AND `items`.`last_time` >= (NOW() - INTERVAL 180 DAY) AND
